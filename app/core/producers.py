...
# from core.events import MARGIN_REQUEST, FORCED_SELL
...
#     @classmethod
#     # def send_event(cls, topic: str, key: str, event: dict):
#     #     """Send event to Kafka"""
#     #     try:
...
#             logger.info(f"📤 Sent event to {topic} | partition={result.partition} offset={result.offset}")

#             # ✅ Save AuditLog immediately
#             AuditLog.objects.create(
#                 action=event.get("type", "UNKNOWN"),
#                 details={"topic": topic, "event": event, "partition": result.partition, "offset": result.offset},
#             )
...
# # def publish_margin_request(client_id: int, amount: float):
# #     event = {
# #         "type": "MARGIN_REQUEST",
# #         "client_id": client_id,
# #         "amount": amount,
...
# #         "margin-loan-events", key=str(client_id), event=event
# #     )



# def publish_margin_request(client_id: int, amount: float):
#     event = {"type": MARGIN_REQUEST, "client_id": client_id, "amount": amount}
#     return KafkaProducerWrapper.send_event("margin-loan-events", key=str(client_id), event=event)
...
#     }
#     return KafkaProducerWrapper.send_event(
#         "portfolio-events", key=str(client_id), event=event
#     )
